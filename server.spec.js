...
describe("server.js", () => {
  ...
  describe("POST: (/games)", () => {
    ...
    it("should return the new games array when correct data is provided", async () => {
      const expected =
        '[{"title":"Pacman","genre":"Arcade","releaseYear":"1980"},{"title":"Borderlands 2","genre":"Looter-Shooter"},{"title":"Warframe","genre":"Looter-Shooter"}]';
      const res = await request(server)
        .post("/games")
        .send({
          title: "Warframe",
    ...
    it("should return appropriate error message when incomplete data is passed", async () => {
      const expected =
        '{"Error":"Game Over Man! A game needs a title and genre to enter the database"}';
      const res = await request(server)
        .post("/games")
        .send({ title: "Borderlands 2" });
      expect(res.text).toEqual(expected);
